...
async function signIn(page: Page, role: "coach" | "player") {
  ...
  const account = page.getByTestId(`account-${role}`);
  await expect(account).toBeVisible();

  // The login page is prerendered, so the button exists before React hydrates
  // and attaches its handler. Retry until the click actually takes.
  await expect(async () => {
    await account.click();
...
test.describe("coach", () => {
  test("opens an athlete and reads their session", async ({ page }) => {
    ...
    await expect(page.getByText(/pitches faced/)).toBeVisible();
